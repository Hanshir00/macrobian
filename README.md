...
to GitHub via Keystatic Cloud's OAuth layer.

1. Go to [keystatic.cloud](https://keystatic.cloud) → Sign in with GitHub.
2. Create a new team and project. Connect it to the GitHub repo for this
   site.
3. Keystatic Cloud will give you three values to set as **environment
   variables in Netlify** (Site settings → Environment variables):

   ```
   KEYSTATIC_GITHUB_CLIENT_ID=<from Keystatic Cloud>
   KEYSTATIC_GITHUB_REPO_OWNER=<your GitHub username>
   KEYSTATIC_GITHUB_REPO_NAME=<the repo name>
   ```

4. Trigger a redeploy (Deploys → Trigger deploy → Clear cache and deploy
   site). After it finishes, `https://yoursite.com/keystatic` will let you
   sign in with GitHub and start editing.

> **Free tier:** Keystatic Cloud is free for up to 3 team members. For solo
> use you're well under the limit.
...
- `/keystatic` redirects to GitHub login on production but never returns →
  check the three `KEYSTATIC_GITHUB_*` env vars in Netlify.
