...
/* =========================================================
   STORAGE
   --------------------------------------------------------
   Local mode in dev (writes to disk). GitHub mode in production
   when KEYSTATIC_GITHUB_CLIENT_ID is set (via Keystatic Cloud).
   ========================================================= */
...
const storage = process.env.KEYSTATIC_GITHUB_CLIENT_ID
  ? ({
      kind: 'github',
      repo: {
        owner: process.env.KEYSTATIC_GITHUB_REPO_OWNER ?? 'YOUR-USERNAME',
        name:  process.env.KEYSTATIC_GITHUB_REPO_NAME  ?? 'sanguine',
      },
    } as const)
  : ({ kind: 'local' } as const);
...
const pageBlocks = {
  hero: fields.object(
    {
      eyebrow:  fields.text({ label: 'Eyebrow (small caps)', validation: { isRequired: false } }),
      title:    fields.text({ label: 'Title', validation: { isRequired: true } }),
      tagline:  fields.text({ label: 'Tagline', multiline: true, validation: { isRequired: false } }),
      align:    fields.select({
        label: 'Alignment',
        options: [
  ...
  sectionList: fields.object(
    {
      title: fields.text({ label: 'Section title (eyebrow)', validation: { isRequired: false } }),
      items: fields.array(
        fields.object({
          label:       fields.text({ label: 'Label', validation: { isRequired: true } }),
          description: fields.text({ label: 'Description', validation: { isRequired: false } }),
...
export default config({
  ...
  ui: {
    brand: { name: 'Sanguine' },
    navigation: {
      'Site': ['settings', 'pageHome', 'pageSanguine', 'pageContact'],
